...
pointAt(gimbalrxSat1,gs1);
pointAt(gimbaltxSat1,sat2);
pointAt(gimbalrxSat2,sat1);
pointAt(gimbaltxSat2,gs2);
pointAt(gimbalgs2,sat2);


numSat = 2; % Set the desired number of satellites
s = [sat2];     % Initialize the array

for i = 1:numSat
    logicVal = generateRandomBinary();
    if logicVal == 1
        % Add the sat2 to the array 's'
        s = [s, eval(['sat', num2str(1)])];
    end
end
